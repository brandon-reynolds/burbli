...
export default function JobForm({ onCreated }:{ onCreated:(j:Job)=>void }) {
  ...
  // Cost: default is EXACT, “Prefer not to say” last option
  ...
  async function submit() {
    // Required: title, who did it, location (suburb, state, postcode)
    ...
    // Basic cost validation when visible
    ...
    // success -> parent shows green success; no extra toast here
  ...
  return (
    <div className="rounded-3xl border p-6 bg-white">
      ...
      {/* Cost (optional) — moved under Location; Exact default, Prefer not to say last */}
