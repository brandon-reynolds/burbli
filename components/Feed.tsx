  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <article key={j.id} className="rounded-2xl border bg-white p-4">
            <div className="flex items-center justify-between">
              <h3 className="font-medium">
                <Link className="hover:underline" href={`/post/${j.id}`}>
                  {j.title}
                </Link>
              </h3>
              <span
                className={`text-xs px-2 py-1 rounded ${
                  j.recommend ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"
                }`}
              >
            ...
            <div className="mt-1 text-sm">
              Done by <span className="font-medium">{j.business_name}</span>
            </div>
            {j.cost_type !== "hidden" && (
              <div className="mt-1 text-sm">
                Cost:{" "}
        ...
        {filtered.length === 0 && (
          <div className="text-sm text-gray-500">No jobs match your filters yet.</div>
        )}
      </div>
    </div>
  );
