...
export async function GET(req: Request) {
  ...
  if (diag) {
    return NextResponse.json({
      ...
      envNameExpected: "GOOGLE_MAPS_API_KEY",
    });
  }

  if (!hasKey) {
    return NextResponse.json(
      { ok: false, error: "Missing GOOGLE_MAPS_API_KEY on server" },
      { status: 500 }
    );
  }
  ...
  try {
    if (q) {
      const target = `${API}/autocomplete/json?input=${encodeURIComponent(q)}&components=country:${COUNTRY}&key=${key}`;
        ...
        return NextResponse.json(
          {
            ...
            target, // helps us verify query shape
          },
          { status: 502 }
        );
      }
    ...
    if (placeId) {
      const target = `${API}/details/json?place_id=${encodeURIComponent(placeId)}&fields=address_component,name&key=${key}`;
      ...
      const state = STATE_ABBR[stateLong] ?? (stateComp?.short_name as any) ?? "";
      return NextResponse.json({
        ok: true,
